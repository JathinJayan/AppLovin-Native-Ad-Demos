package com.applovin.carouselui;

/**
 * Created by mszaro on 5/27/15.
 */
public class AppLovinCarouselViewSettings
{
    public static final int     NUM_ADS_TO_AUTOLOAD                       = 3;
    public static final int     VIEW_PAGER_MARGIN                         = 10;
    public static final int     VIEW_PAGER_OFF_SCREEN_PAGE_LIMIT          = 2;
    public static final int     VIEW_PAGER_BACKGROUND_COLOR               = android.R.color.white;
    public static final boolean TAP_TO_PAUSE_VIDEO                        = false;
    public static final int     VIDEO_VIEW_BACKGROUND_COLOR               = android.R.color.transparent;
    public static final int     VIDEO_VIEW_BACKGROUND_COLOR_WHILE_PLAYING = android.R.color.transparent;
    public static final boolean USE_VIDEO_SCREENSHOTS_AS_IMAGES           = true;
    public static final int     MAIN_IMAGE_MAX_SCALE_SIZE                 = 350;
    public static final int     ICON_IMAGE_MAX_SCALE_SIZE                 = 50;
    public static final boolean VIDEO_MUTED_BY_DEFAULT                    = true;
    public static final boolean MUTE_FADES_AUDIO                          = true;
    public static final float   VIEW_PAGER_CARD_WIDTH                     = 0.9f;
}
